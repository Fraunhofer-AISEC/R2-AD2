...
# Configuration
this_parse = ArgumentParser(description="Train R2-AD2 on CovType")
add_standard_arguments(this_parse)
this_args = this_parse.parse_args()

experiment_config = [
    CovType(
        random_state=this_args.random_seed, y_normal=[1, 2, 3], y_anomalous=[4, 5, 6, 7],
        n_train_anomalies=this_args.n_train_anomalies, p_pollution=this_args.p_pollution
    ),
    CovType(
        random_state=this_args.random_seed, y_normal=[1, 2, 3], y_anomalous=[4, 5, 6, 7], y_anomalous_train=[4, 5],
        n_train_anomalies=this_args.n_train_anomalies, p_pollution=this_args.p_pollution
    )
]
